...
from pylab import os
import theano.tensor as T
import random
import pickle


def dataresize(path=r'C:\Users\puppy\face\model\images\test'):  # test path
    path_t =r"C:\Users\puppy\face\model\images\train"  # train path
    datas = []
    train_x= []
    train_y= []
    valid_x= []
    ...
    for num in range(len(datas)):
        ...
        datas[num] =(datas[num])[1:]  #将数据的标签项去掉
    tests = []  # #读取测试集
    for dirs in os.listdir(path_t):
        for filename in os.listdir(os.path.join(path_t,dirs)):
            ...
            width,hight=img.size
            img = numpy.asarray(img,dtype='float64')/256.
            tmp = img.reshape(1, hight*width)[0]  # 在此如果不是取出[0]的话在后面会发现其实其是一个多维的数据的叠加，
    ...
    for num in range(len(tests)):
        ...
        tests[num] =(tests[num])[1:]  #将数据的标签项去掉
...
if __name__=='__main__':  dataresize()
